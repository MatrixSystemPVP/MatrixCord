  ...
  async run(msg: CommandoMessage, args: any) {
    ...
    if (this.client.shard) {
      try {
        ...
        await msg.reply(createEmbed(`Unloaded \`${args.command.name}\` command, but failed to unload on other shards.`))
        return null
      }
    }

    await msg.reply(createEmbed(`Unloaded \`${args.command.name}\` command${this.client.shard ? ' on all shards' : ''}.`))
    return null
  }
}
